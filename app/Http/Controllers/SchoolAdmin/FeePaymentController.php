...
class FeePaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'method'           => 'required|in:cash,card,online,orange_money,afrimoney,mobile_money',
        ...
        // For mobile money, initiate payment first
        if (in_array($data['method'], ['orange_money', 'afrimoney', 'mobile_money'])) {
            $phone = $data['phone'] ?? $request->phone;
            if (blank($phone)) {
                return back()->withInput()->with('error', 'Phone number is required for mobile money payments.');
            }

            $reference = 'RCP-' . now()->format('Ymd-His') . '-' . rand(1000, 9999);
            $gateway = new PaymentGatewayService($sid);
            $result = $gateway->initiatePayment($data['method'], $phone, $amountPaid, $reference, "School fees payment");

            // Record as pending
            $payment = FeePayment::create(array_merge($data, [
                'school_id' => $sid,
                'discount'  => $discount,
                'fine'      => $fine,
                'status'    => 'pending',
                'receipt_no' => $reference,
                'note'      => ($data['note'] ?? '') . " [{$data['method']} initiated]",
            ]));

            if ($result['success']) {
                $payment->update(['transaction_ref' => $result['transaction_id'] ?? $result['pay_token'] ?? null]);
                return back()->with('success', 'Payment initiated. Redirecting to payment gateway...')->with('payment_url', $result['payment_url'] ?? null);
            }

            return back()->withInput()->with('error', 'Mobile money payment failed: ' . ($result['error'] ?? 'Unknown error'));
        }
