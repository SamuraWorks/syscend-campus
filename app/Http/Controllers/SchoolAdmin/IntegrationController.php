...
class IntegrationController extends Controller
{
    ...
    public function testSmtp(Request $request)
    {
        ...
        try {
            ...
            Mail::raw('This is a test email from Syscend Campus to verify your SMTP configuration.', function ($msg) use ($request, $settings) {
                $msg->to($request->test_email)
                    ->subject('Syscend Campus — SMTP Test')
                    ->from($settings['smtp_from_email'] ?? config('mail.from.address'), $settings['smtp_from_name'] ?? 'Syscend Campus');
            });
    ...
    public function testSms(Request $request)
    {
        ...
        try {
            $sms = new SmsService($sid);
            $result = $sms->send($request->test_phone, 'Test SMS from Syscend Campus. Your SMS gateway is configured correctly.');

            if ($result['success']) {
                return back()->with('success', 'Test SMS sent to ' . $request->test_phone);
            }

            return back()->with('error', 'SMS test failed: ' . ($result['error'] ?? 'Unknown error'));
        } catch (\Throwable $e) {
