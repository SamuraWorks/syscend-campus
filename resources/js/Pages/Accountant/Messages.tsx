...
export default function AccountantMessages({ linked, inbox, sent, users }: Props) {
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post('/school/accountant/messages/send', { onSuccess: () => { reset(); setComposing(false); } });
    }
                        ...
                        {inbox.length === 0 ? (
                            ...
                            inbox.map(msg => (
                                <Card
                                    key={msg.id}
                                    className={cn(
                                        !msg.is_read && 'border-l-4 border-l-emerald-500 bg-emerald-50/30 dark:bg-emerald-950/10'
                                    )}
                                >
                                    <CardContent className="p-5">
                                        <div className="flex items-start gap-3">
                                            <div className={cn(
                                                ...
                                                <Mail className={cn('w-4 h-4', msg.is_read ? 'text-slate-500' : 'text-emerald-600 dark:text-emerald-400')} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between gap-2 mb-1">
                                                    ...
                                                    {!msg.is_read && <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />}
                                                </div>
