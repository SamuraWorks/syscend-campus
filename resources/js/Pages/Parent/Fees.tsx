                    ...
                    <div key={child.id}>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                            ...
                            <Card><CardContent className="p-3 text-center">
                                ...
                                <p className="font-bold text-green-600">Le {child.total_paid.toLocaleString()}</p>
                            </CardContent></Card>
                            <Card className={child.balance > 0 ? 'border-red-200' : 'border-green-200'}>
                                <CardContent className="p-3 text-center">
                                    ...
                                    <p className={cn('font-bold', child.balance > 0 ? 'text-red-600' : 'text-green-600')}>
                                        {child.balance > 0 ? `Le ${child.balance.toLocaleString()}` : 'Clear'}
                                    </p>
                                </CardContent>
                            </Card>
                        </div>
                                        ...
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                            {child.payments.map((p, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="px-4 py-2 text-right text-slate-600">Le {p.due.toLocaleString()}</td>
                                                    <td className="px-4 py-2 text-right text-green-600">Le {p.paid.toLocaleString()}</td>
