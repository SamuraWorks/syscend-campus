...
import {
    User, GraduationCap, Target, BookOpen, ClipboardCheck, HandHeart, SmilePlus, Award,
    AlertTriangle, Shield, TrendingUp, BarChart3, Star, CheckCircle,
} from 'lucide-react';
...
const gaugeColors: Record<string, string> = {
    ...
    good: 'text-blue-500',
    needs_monitoring: 'text-yellow-500',
    needs_support: 'text-orange-500',
...
export default function StudentProfile({ linked, profile }: Props) {
    if (!linked) {
        return (
            <AppLayout title="Student Profile">
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-md">Link your account to a school to view your performance profile.</p>
                </div>
            </AppLayout>
        );
    }

    const { student, current_score, goals, achievements, pending_alerts, active_interventions } = profile;
    ...
    return (
        <AppLayout title="Student Performance Profile">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card>
                        <CardContent className="p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-lg font-bold text-slate-900 dark:text-white">{pending_alerts}</p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Pending Alerts</p>
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-lg font-bold text-slate-900 dark:text-white">{active_interventions}</p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Active Interventions</p>
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-lg font-bold text-slate-900 dark:text-white">
                                    {score?.class_rank ? `#${score.class_rank}` : '—'}
                                </p>
                    ...
                    <Card>
                        <CardContent className="p-4 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-lg font-bold text-slate-900 dark:text-white">
                                    {score?.school_rank ? `#${score.school_rank}` : '—'}
                                </p>
