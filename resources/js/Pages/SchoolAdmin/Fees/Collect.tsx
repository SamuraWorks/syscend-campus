                        ...
                        <CardContent>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                        ...
                                        <SelectContent>
                                            {structures.map(s => (
                                                <SelectItem key={s.id} value={String(s.id)}>
                                                    {s.fee_category?.name} — Le {Number(s.amount).toLocaleString()} ({s.frequency}) · {s.academic_year}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div className="space-y-1.5">
                                        <Label>Amount Due (Le) <span className="text-red-500">*</span></Label>
                                ...
                                <div className="rounded-lg bg-slate-50 dark:bg-slate-900 px-4 py-3 flex items-center justify-between">
                                    ...
                                    <div className="font-bold text-lg text-slate-900 dark:text-white">
                                        Le {(Number(data.amount_due || 0) + Number(data.fine || 0) - Number(data.discount || 0)).toLocaleString()}
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div className="space-y-1.5">
                                        <Label>Amount Paid (Le) <span className="text-red-500">*</span></Label>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className={`h-10 rounded-md border px-3 flex items-center font-semibold ${balance > 0 ? 'text-red-600 border-red-200 bg-red-50 dark:bg-red-950/20' : 'text-green-600 border-green-200 bg-green-50 dark:bg-green-950/20'}`}>
                                            {balance > 0 ? `Le ${balance.toLocaleString()}` : 'Fully Paid'}
                                        </div>
                                    </div>
                                </div>
