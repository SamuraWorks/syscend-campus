...
export default function FeePayments({ payments, classes, filters, stats }: Props) {
    ...
    const statCards = [
        { label: 'Total Collected', value: `Le ${stats.total_collected?.toLocaleString() ?? 0}`, color: 'text-green-600', icon: DollarSign },
        { label: 'Outstanding', value: `Le ${Math.max(0, stats.total_outstanding ?? 0).toLocaleString()}`, color: 'text-red-600', icon: TrendingDown },
                            ...
                            {payments.data.length === 0 ? (
                                ...
                                return (
                                    <TableRow key={p.id}>
                                        ...
                                        <TableCell className="hidden lg:table-cell text-right text-sm">Le {Number(p.amount_due).toLocaleString()}</TableCell>
                                        <TableCell className="hidden lg:table-cell text-right text-sm font-semibold text-green-600">Le {Number(p.amount_paid).toLocaleString()}</TableCell>
                                        <TableCell className={`hidden md:table-cell text-right text-sm font-medium ${Number(balance) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                            {Number(balance) > 0 ? `Le ${Number(balance).toLocaleString()}` : '—'}
                                        </TableCell>
                                        <TableCell className="hidden md:table-cell text-xs text-slate-500">{METHOD_LABELS[p.method] ?? p.method}</TableCell>
