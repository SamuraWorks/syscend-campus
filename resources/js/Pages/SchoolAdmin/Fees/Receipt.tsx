                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-slate-900 dark:text-white">Le {Number(payment.amount_due).toLocaleString()}</span>
                            </div>
                            {Number(payment.fine) > 0 && (
                                <div className="flex justify-between text-sm">
                                    <span className="text-red-500">Fine</span>
                                    <span className="text-red-500">+ Le {Number(payment.fine).toLocaleString()}</span>
                                </div>
                            )}
                            {Number(payment.discount) > 0 && (
                                <div className="flex justify-between text-sm">
                                    <span className="text-green-500">Discount</span>
                                    <span className="text-green-500">- Le {Number(payment.discount).toLocaleString()}</span>
                                </div>
                            )}
                            <div className="border-t border-slate-100 dark:border-slate-800 pt-2 flex justify-between font-semibold">
                                ...
                                <span className="text-slate-900 dark:text-white">
                                    Le {(Number(payment.amount_due) + Number(payment.fine) - Number(payment.discount)).toLocaleString()}
                                </span>
                            </div>
                                <div className="flex justify-between font-bold text-lg">
                                <span className="text-slate-700 dark:text-slate-300">Amount Paid</span>
                                <span className="text-green-600">Le {Number(payment.amount_paid).toLocaleString()}</span>
                            </div>
                            {balance > 0 && (
