        ...
        <AppLayout title="Payroll">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-lg">
                    {[
                        { label: 'Net Payable', value: `Le ${stats.total_net?.toLocaleString() ?? 0}`, color: 'text-indigo-600', icon: DollarSign },
                        ...
                        <TableBody>
                            {payrolls.data.length === 0 ? (
                                ...
                                <TableRow key={p.id}>
                                    ...
                                    <TableCell className="hidden md:table-cell text-right text-sm">Le {Number(p.basic_salary).toLocaleString()}</TableCell>
                                    <TableCell className="hidden md:table-cell text-right text-sm text-green-600">Le {Number(p.total_allowances).toLocaleString()}</TableCell>
                                    <TableCell className="hidden md:table-cell text-right text-sm text-red-600">Le {Number(p.total_deductions).toLocaleString()}</TableCell>
                                    <TableCell className="text-right font-bold text-indigo-600">Le {Number(p.net_salary).toLocaleString()}</TableCell>
                                    <TableCell className="hidden md:table-cell text-center text-sm text-slate-500">{p.present_days}/{p.working_days}</TableCell>
                                    <TableCell>
