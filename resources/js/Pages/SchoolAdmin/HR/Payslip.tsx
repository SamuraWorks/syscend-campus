                ...
                <div id="payslip-print-area" className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <div className="space-y-1.5">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium text-slate-900 dark:text-white">Le {Number(payroll.basic_salary).toLocaleString()}</span>
                                </div>
                                {(payroll.allowances_snapshot ?? []).map((a, i) => (
                                    <div key={i} className="flex justify-between text-sm">
                                        <span className="text-slate-500">{a.label}</span>
                                        <span className="text-green-600">Le {Number(a.amount).toLocaleString()}</span>
                                    </div>
                                ))}
                                <div className="flex justify-between text-sm font-semibold border-t border-slate-100 dark:border-slate-800 pt-1">
                                    ...
                                    <span>Le {(Number(payroll.basic_salary) + Number(payroll.total_allowances)).toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="space-y-1.5">
                                    {(payroll.deductions_snapshot ?? []).map((d, i) => (
                                        <div key={i} className="flex justify-between text-sm">
                                            ...
                                            <span className="text-red-600">- Le {Number(d.amount).toLocaleString()}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="rounded-lg bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-800 px-4 py-3 flex justify-between items-center">
                            ...
                            <span className="text-2xl font-bold text-indigo-600">Le {Number(payroll.net_salary).toLocaleString()}</span>
                        </div>
