                            ...
                            {staffList.data.map(s => {
                                ...
                                return (
                                    <TableRow key={s.id}>
                                        ...
                                        <TableCell className="text-right text-sm">{struct ? `Le ${Number(struct.basic_salary).toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right text-sm text-green-600">{struct ? `Le ${allowTotal.toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right text-sm text-red-600">{struct ? `Le ${dedTotal.toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right font-bold text-indigo-600">{netSal != null ? `Le ${netSal.toLocaleString()}` : <Badge className="bg-amber-100 text-amber-700 border-0 text-xs">Not Set</Badge>}</TableCell>
                                        <TableCell>
                ...
                <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-5 mt-2">
                        <div className="space-y-1.5">
                            <Label>Basic Salary (Le) <span className="text-red-500">*</span></Label>
                        ...
                        <div className="rounded-lg bg-slate-50 dark:bg-slate-900 p-3 space-y-1 text-sm">
                            <div className="flex justify-between"><span className="text-slate-500">Gross Salary</span><span className="font-medium">Le {grossSalary.toLocaleString()}</span></div>
                            <div className="flex justify-between text-red-600"><span>Total Deductions</span><span>- Le {totalDeductions.toLocaleString()}</span></div>
                            <div className="flex justify-between font-bold text-base border-t border-slate-200 dark:border-slate-700 pt-1 mt-1">
                                <span>Net Salary</span><span className="text-indigo-600">Le {netSalary.toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                    <DialogFooter>
