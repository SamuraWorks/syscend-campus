    ...
    return (
        <AppLayout title="Homework">
                            ...
                            <TableBody>
                                ...
                                {homework.data.map(hw => (
                                    <TableRow key={hw.id}>
                                        ...
                                        <TableCell className="hidden md:table-cell">{hw.subject?.name ?? '—'}</TableCell>
                                        <TableCell className="hidden md:table-cell">{hw.teacher ? `${hw.teacher.first_name} ${hw.teacher.last_name}` : '—'}</TableCell>
            ...
            <ConfirmDialog
                ...
                description={`Assign homework "${data.title || 'Untitled'}"? All students in the selected class will be notified.`}
                confirmText="Assign Homework"
                onConfirm={() => {
                    post('/school/homework', { onSuccess: () => { reset(); setOpen(false); setShowConfirm(false); } });
                }}
            />
        </AppLayout>
    );
