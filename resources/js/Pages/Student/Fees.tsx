                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card>
                        <CardContent className="p-4 text-center">
                            ...
                            <p className="text-lg font-bold text-slate-800 dark:text-white">Le{summary.total_due.toLocaleString()}</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4 text-center">
                            <p className="text-xs text-slate-500 mb-1">Total Paid</p>
                            <p className="text-lg font-bold text-green-600">Le{summary.total_paid.toLocaleString()}</p>
                        </CardContent>
                    </Card>
                    <Card className={cn(summary.balance > 0 ? 'border-red-200 dark:border-red-900' : 'border-green-200 dark:border-green-900')}>
                        <CardContent className="p-4 text-center">
                            ...
                            <p className={cn('text-lg font-bold', summary.balance > 0 ? 'text-red-600' : 'text-green-600')}>
                                {summary.balance > 0 ? `Le${summary.balance.toLocaleString()}` : 'Clear'}
                            </p>
                        </CardContent>
                    </Card>
                </div>
                    ...
                    <CardContent className="p-0">
                        {payments.length === 0 ? (
                            <div className="py-12 text-center">
                                <DollarSign className="w-10 h-10 text-slate-200 dark:text-slate-700 mx-auto mb-2" />
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {payments.map(p => (
                                        <tr key={p.id}>
                                            ...
                                            <td className="px-4 py-2.5 text-right text-slate-600">Le{p.due.toLocaleString()}</td>
                                            <td className="px-4 py-2.5 text-right text-green-600">Le{p.paid.toLocaleString()}</td>
                                            <td className={cn('px-4 py-2.5 text-right font-medium', p.balance > 0 ? 'text-red-500' : 'text-green-600')}>
                                                {p.balance > 0 ? `Le${p.balance.toLocaleString()}` : '—'}
                                            </td>
