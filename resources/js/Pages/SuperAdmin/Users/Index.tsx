...
interface UserRow {
    ...
    status: string; school_id: number | null; roles: Role[];
...
interface Props {
    ...
    schools: School[];
    roles: string[];
    filters: { search?: string; role?: string; school_id?: string; status?: string };
}
...
export default function UsersIndex({ users, schools, roles, filters }: Props) {
    ...
    function applyFilters(overrides: Record<string, string> = {}) {
        router.get('/super-admin/users', {
            search: search, role: roleFilter, school_id: schoolFilter,
            status: statusFilter, ...overrides,
    ...
    function confirmDelete() {
        ...
        router.delete(`/super-admin/users/${deleteUser.id}`, { onSuccess: () => setDeleteUser(null) });
    }

    function toggleStatus(u: UserRow) {
        const action = u.status === 'suspended' ? 'activate' : 'suspend';
        router.patch(`/super-admin/users/${u.id}/${action}`);
    }
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-0.5">Manage all system users, roles, and access</p>
                    </div>
                                        ...
                                        <tr key={u.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            ...
                                            <td className="py-3 px-4">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    <button onClick={() => toggleStatus(u)} title={u.status === 'suspended' ? 'Activate' : 'Suspend'}
                ...
                <DialogContent className="max-w-sm">
                    ...
                    <form onSubmit={submitResetPassword} className="space-y-4">
                        <div>
                            ...
                            <Input type="password" value={resetForm.data.password}
                                onChange={e => resetForm.setData('password', e.target.value)}
                                placeholder="Min 8 characters" />
                            {resetForm.errors.password && <p className="text-xs text-red-500 mt-1">{resetForm.errors.password}</p>}
                        </div>
                        <DialogFooter>
