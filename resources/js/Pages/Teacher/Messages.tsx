...
import { Mail, Clock, User, Send, Inbox, ArrowUpRight } from 'lucide-react';
...
interface Props {
    ...
    inbox: InboxMsg[]; sent: SentMsg[]; users: UserEntry[];
}

export default function TeacherMessages({ linked, inbox, sent, users }: Props) {
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post('/school/teacher/messages/send', { onSuccess: () => { reset(); setComposing(false); } });
    }
        ...
        return (
            <AppLayout title="Messages">
                <div className="flex flex-col items-center justify-center py-24 text-center">
                    <Mail className="w-14 h-14 text-slate-300 mb-4" />
                            ...
                            <form onSubmit={submit} className="space-y-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                                    >
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 resize-none"
                                    />
                ...
                <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700">
                    ...
                    >
                        ...
                        {inbox.filter(m => !m.is_read).length > 0 && (
                            <Badge className="ml-2 text-xs bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400">{inbox.filter(m => !m.is_read).length}</Badge>
                        )}
                    </button>
                    <button
                        onClick={() => setTab('sent')}
